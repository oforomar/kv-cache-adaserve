...
- **`score()` formula sign**: the design doc gives `score = -(Δppl) - λ·(1 - cratio)`, but its accompanying text says λ=10 favors aggressive compression. The formula as written *penalizes* compression at high λ (smoke-tested — at λ=10 the highest-cratio strategy wins ~all prompts). Either the formula sign is wrong (should be `+ λ·(1 - cratio)`) or the description is. The code in `strategies.score()` follows the doc's formula verbatim; resolve before any production label run.
